...
const normalizeDefinition = function <T extends DeviceTypeConfigurationConfig>(
	definition: T,
) {
	const result = {
		...definition,
...
export type DeviceTypeConfigurationConfigWithImage =
	DeviceTypeConfigurationConfig &
		Required<Pick<DeviceTypeConfigurationConfig, 'image'>>;
...
export const commands = {
	copy(image: string, operation: CopyOperation) {
		...
		const fromDefinition = normalizeDefinition(
			operation.from as typeof operation.from &
				Required<Pick<typeof operation.from, 'image'>>,
		);
		const toDefinition = normalizeDefinition(
			operation.to as typeof operation.to &
	...
	replace(image: string, operation: ReplaceOperation) {
		...
		const fileDefinition = normalizeDefinition(
			operation.file as typeof operation.file &

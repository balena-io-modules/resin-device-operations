...
wary.it(
	'should be able to copy a single file between raspberry pi partitions',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configuration = operations.execute(images.raspberrypi, [
			{
...
wary.it(
	'should be able to replace a single file from a raspberry pi partition',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configuration = operations.execute(images.raspberrypi, [
			{
		...
		return rindle
			...
			.then((contents) =>
				expect(contents).to.equal(
					'dwc_otg.lpm_enable=1 console=ttyAMA0,115200 kgdboc=ttyAMA0,115200 root=/dev/mmcblk0p2 rootfstype=ext4 rootwait \n',
				),
			);
	},
);
...
wary.it(
	'should be able to perform multiple replaces in an raspberry pi partition',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configuration = operations.execute(images.raspberrypi, [
			{
		...
		return rindle
			...
			.then((contents) =>
				expect(contents).to.equal(
					'dwc_otg.lpm_enable=2 console=ttyAMA0,115200 kgdboc=ttyAMA0,115200 root=/dev/mmcblk0p2 rootfstype=ext4 rootwait \n',
				),
			);
	},
);
...
wary.it(
	'should be able to completely replace a file from an edison partition',
	{ edison: EDISON },
	function (images) {
...
wary.it(
	'should obey when properties',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configuration = operations.execute(
			images.raspberrypi,
			[
				{
					command: 'replace',
...
wary.it(
	'should emit state events for operations',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configuration = operations.execute(images.raspberrypi, [
			{
...
wary.it(
	'should read state events for operations after a slight delay',
	{ raspberrypi: RASPBERRY_PI },
	function (images) {
		const configure = () =>
			Promise.try(() =>
...
wary.it(
	'should run a script with arguments that exits successfully',
	{},
	function () {
...
wary.it(
	'should run a script that doesn not have execution privileges',
	{},
	function () {
		const configuration = operations.execute(EDISON_ZIP, [
			{
...
wary.it(
	'should be rejected if the script finishes with an error',
	{},
	function () {
		const configuration = operations.execute(EDISON_ZIP, [
			{
...
wary.it(
	'should change directory to the dirname of the script',
	{},
	function () {
		const configuration = operations.execute(EDISON_ZIP, [
			{
...
wary.it(
	'should be rejected if the burn operation lacks a drive option',
	{},
	function () {
		const configuration = operations.execute(RASPBERRY_PI, [
			{ command: 'burn' },
...
wary.it(
	'should be able to burn an image',
	{
		raspberrypi: RASPBERRY_PI,
		random: RANDOM,
	},
...
wary.it(
	'should set an os option automatically',
	{ edison: EDISON },
	function (images) {
		const configuration = operations.execute(images.edison, [
			{
		...
		return rindle
			...
			.then((contents) =>
				expect(contents).to.equal(utils.getOperatingSystem()),
			);
	},
);

wary.it(
	'should allow the os option to be overrided',
	{ edison: EDISON },
	function (images) {
		const configuration = operations.execute(
			images.edison,

...
describe('Utils:', function () {
	describe('.filterWhenMatches()', function () {
		describe('given operations without a when property', function () {
			beforeEach(function () {
				return (this.operations = [{ command: 'foo' }, { command: 'bar' }]);
			});

			it('should return the same operations', function () {
				expect(utils.filterWhenMatches(this.operations)).to.deep.equal(
					this.operations,
				);
			});
		});
		...
		describe('given operations with a when property', function () {
			...
			it('should return the operatiosn what match the options', function () {
				const operations = utils.filterWhenMatches(this.operations, {
					hello: 'planet',
				});
	...
	describe('.getOperatingSystem()', function () {
		describe('given darwin', function () {
			...
			it('should return osx', () =>
				expect(utils.getOperatingSystem()).to.equal('osx'));
		});
		...
		describe('given win32', function () {
			...
			it('should return win32', () =>
				expect(utils.getOperatingSystem()).to.equal('win32'));
		});
		...
		describe('given linux', function () {
			...
			it('should return linux', () =>
				expect(utils.getOperatingSystem()).to.equal('linux'));
		});
	});
});
